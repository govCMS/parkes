...
// To understand behaviors, see https://drupal.org/node/756722#behaviors
  Drupal.behaviors.tofuSiteNavigation = {
    ...
    init: function() {
      ...
      $('.govcms-parkes-global-nav .govcms-parkes-global-nav-sub-menu ul').prepend(close);
      // $('.govcms-parkes-global-nav nav a').materialripple();
      this.applyBorderStyling();
    },
    ...
    closeMenu: function() {
      this.slide('right', this.positionInitial, function() {
        ...
        $('.govcms-parkes-global-nav .border-left').removeClass('border-left');
        $('.govcms-parkes-global-nav .border-right').removeClass('border-right');
      });
    },
    ...
    toggleMenu: function() {
      $('#govcms-parkes-global-nav-toggle a').toggleClass('open');
      if (this.isOpen()) {
        this.closeMenu();
      }
      else {
    ...
    // Determines if the moduile menu it open or not
    isOpen: function() {
    ...
    // Applies border styling to the menus.
    applyBorderStyling: function() {

      // All of the site sub-menus sit next to each other.  If we apply and left
      // and right borders to both, we'll get double borders which will look
      // ugly.
      var menus = $('.govcms-parkes-global-nav ul');

      // Apply a left border to the first menu.
      // $(menus[0]).addClass('border-left');

      // We need to apply a border to the longest menu where two menus
      // meet.
      // @todo
